...
def verifca(prueba, cont, i, j, tamanio):
    c = 0
    cantperm = pow(tamanio, 2)*cont #Cantidad de permutaciones
...
def is_posible(prueba, objetivo, i, j, tamanio, listaThread):
    ...
    h = j
    lstOptima = []
    while(h < tamanio and validarAmp(matrizF[i][h]) == False):
        cont += 1 #Cont de la cantidad de casillas
        h += 1
    for opt in lst:
        if(len(str(opt)) == cont):
    ...
    for row, col in product(range(tamanio), repeat=2):
        ...
        if (validarAmp(brd[row][col]) == False):
            if(int(brd[row][col]) == 0):
                ...
                listaThreads = [listaThread, listaThread2, listaThread3, listaThread4, listaThread5, listaThread6,
                                listaThread7, listaThread8, listaThread9]
                # for prueba in [1,2,3,4,5,6,7,8,9]:

                t1 = threading.Thread(target=is_posible, args=(1, objetivo, row, col, tamanio, listaThread))
                t1.setDaemon(True)
                t1.start()

                t2 = threading.Thread(target=is_posible, args=(2, objetivo, row, col, tamanio, listaThread2))
                t2.setDaemon(True)
                t2.start()
                t3 = threading.Thread(target=is_posible, args=(3, objetivo, row, col, tamanio, listaThread3))
                t3.setDaemon(True)
                t3.start()
                t4 = threading.Thread(target=is_posible, args=(4, objetivo, row, col, tamanio, listaThread4))
                t4.setDaemon(True)
                t4.start()
                t5 = threading.Thread(target=is_posible, args=(5, objetivo, row, col, tamanio, listaThread5))
                t5.setDaemon(True)
                t5.start()
                t6 = threading.Thread(target=is_posible, args=(6, objetivo, row, col, tamanio, listaThread6))
                t6.setDaemon(True)
                t6.start()
                t7 = threading.Thread(target=is_posible, args=(7, objetivo, row, col, tamanio, listaThread7))
                t7.setDaemon(True)
                t7.start()
                t8 = threading.Thread(target=is_posible, args=(8, objetivo, row, col, tamanio, listaThread8))
                t8.setDaemon(True)
                t8.start()
                t9 = threading.Thread(target=is_posible, args=(9, objetivo, row, col, tamanio, listaThread9))
                t9.setDaemon(True)
                t9.start()

                # t2.join()
                # t9.join()
                for prueba in listaThreads:
                    # print(prueba[0])
                    if(prueba[0] == True):
...
def fijos2(tamanio):
    ...
    # tiempofin = time()
    print("Tiempo con hilos = {0:.5f}".format(time() - tiempoini))
